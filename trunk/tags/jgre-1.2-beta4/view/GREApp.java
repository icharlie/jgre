...
public class GREApp extends SingleFrameApplication {
  ...
  protected void startup() {
    ...
    if (osName.startsWith("Mac OS")) {
      try {
        ...
        UIManager.put("MenuBarUI", UIManager.get("MenuBarUI"));
        UIManager.put("MenuUI", UIManager.get("MenuUI"));
        UIManager.put("CheckBoxMenuItemUI", UIManager.get("CheckBoxMenuItemUI"));
        UIManager.put("RadioButtonMenuItemUI", UIManager.get("RadioButtonMenuItemUI"));
        UIManager.put("PopupMenuUI", UIManager.get("PopupMenuUI"));
        UIManager.setLookAndFeel("com.sun.java.swing.plaf.nimbus.NimbusLookAndFeel");
      } catch (Exception ex) {
        Logger.getLogger(GREApp.class.getName()).log(Level.SEVERE, null, ex);
      }
    }
  ...
  protected void initialize(String[] args) {
    ...
    // 設定檔
    File configurationFile = new File(applicationPath.concat(Configure.CONFIGURE_FILE_NAME));
    File databaseFile = new File(applicationPath.concat("DB").concat(File.separator).concat("word.mdb"));
    File voiceDir = new File(applicationPath.concat("TTS").concat(File.separator));
    File wordListFile = new File(applicationPath.concat("TTS").concat(File.separator).concat("wordList999.txt"));
    ...
    if (!configurationFile.exists()) {
      ...
      try {
        Serializer serializer = new Serializer(new FileOutputStream(configurationFile), "UTF8");
